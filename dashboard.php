          ...
          if ($run) {
              if (mysqli_num_rows($run) > 0) {
                  while ($items = mysqli_fetch_assoc($run)) {
                      ...
                      <button onclick="handleCommentClick('<?php echo htmlspecialchars($items['comments']); ?>')"><?php echo htmlspecialchars($items['comments']); ?></button>
                      <?php
                      $rank++;
                  }
              } else {
...
<!-- Add this script at the end of your HTML file, just before </body> -->
<script>
// Function to handle the click event on comments
function handleCommentClick(comment) {
    // AJAX request to fetch coordinates based on the comment
    fetch(`get_coordinates.php?comment=${comment}`)
        ...
        .then(data => {
            ...
            if (data.error) {
                ...
                // Create a Leaflet map and display the location
                const map = L.map('map').setView([latitude, longitude], 13);
                L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
                L.marker([latitude, longitude]).addTo(map)
                    .bindPopup('Location from Comment');
            }
        })
        .catch(error => {
...
// Attach click event listeners to comments
document.addEventListener('DOMContentLoaded', function () {
    const commentElements = document.querySelectorAll('#showcomments > div');
    commentElements.forEach(commentElement => {
        const comment = commentElement.querySelector('div:nth-child(2)').innerText;
